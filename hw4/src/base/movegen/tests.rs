...
fn pawn_capture_generated() {
    ...
    assert!(get_moves::<{ GenMode::All }>(&b).contains(&m));
    assert!(get_moves::<{ GenMode::Captures }>(&b).contains(&m));
}
...
fn king_has_only_one_move() {
    ...
    assert!(has_moves(&b));
    assert!(get_moves::<{ GenMode::All }>(&b).len() == 1);
...
fn pinned_knight_capture() {
    ...
    assert!(!get_moves::<{ GenMode::All }>(&b).contains(&(illegal_move)));
    assert!(!get_moves::<{ GenMode::Captures }>(&b).contains(&(illegal_move)));
...
fn king_can_move() {
    ...
    assert!(!get_moves::<{ GenMode::All }>(&b).is_empty());
    assert!(!get_moves::<{ GenMode::Captures }>(&b).is_empty());
    assert!(!get_moves::<{ GenMode::Quiets }>(&b).is_empty());
    assert!(has_moves(&b));
}
